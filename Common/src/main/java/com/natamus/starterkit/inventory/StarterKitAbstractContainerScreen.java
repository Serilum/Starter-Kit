...
public abstract class StarterKitAbstractContainerScreen<T extends AbstractContainerMenu> extends Screen implements MenuAccess<T> {
    ...
    private void renderSlotHighlightBack(GuiGraphicsExtractor $$0) {
    ...
    private void renderSlotHighlightFront(GuiGraphicsExtractor $$0) {
        if (this.hoveredSlot != null && this.hoveredSlot.isHighlightable()) {
    ...
    protected void renderTooltip(GuiGraphicsExtractor guiGraphics, int i, int j) {
    ...
    private void renderFloatingItem(GuiGraphicsExtractor $$0, ItemStack $$1, int $$2, int $$3, @Nullable String $$4) {
        $$0.item($$1, $$2, $$3);
        $$0.itemDecorations(this.font, $$1, $$2, $$3 - (this.draggingItem.isEmpty() ? 0 : 8), $$4);
    }
    ...
    protected abstract void renderBg(GuiGraphicsExtractor guiGraphics, float partialTick, int mouseX, int mouseY);

    protected void renderSlot(GuiGraphicsExtractor $$0, Slot $$1) {
        int $$2 = $$1.x;
        int $$3 = $$1.y;
        ...
        if ($$1 == this.clickedSlot && !this.draggingItem.isEmpty() && this.isSplittingStack && !$$4.isEmpty()) {
            ...
            if (AbstractContainerMenu.canItemQuickReplace($$1, $$7, true) && this.menu.canDragTo($$1)) {
                ...
                int $$11 = AbstractContainerMenu.getQuickCraftPlaceCount(this.quickCraftSlots.size(), this.quickCraftingType, $$7) + $$10;
                if ($$11 > $$9) {
                    $$11 = $$9;
        ...
        if (!$$6) {
            ...
            $$0.itemDecorations(this.font, $$4, $$2, $$3, $$8);
        }
        ...
        if (!itemStack.isEmpty() && this.isQuickCrafting) {
            if (this.quickCraftingType == 2) {
                ...
                for(Iterator var2 = this.quickCraftSlots.iterator(); var2.hasNext(); this.quickCraftingRemainder -= k - i) {
                    ...
                    k = Math.min(AbstractContainerMenu.getQuickCraftPlaceCount(this.quickCraftSlots.size(), this.quickCraftingType, itemStack) + i, j);
                }
...
public boolean mouseClicked(@NotNull MouseButtonEvent mouseButtonEvent, boolean $$1) {
    if (super.mouseClicked(mouseButtonEvent, $$1)) {
        ...
        if (mouseButtonEvent.button() != 0 && mouseButtonEvent.button() != 1 && !$$2) {
            ...
            if ($$7 != -1 && !this.isQuickCrafting) {
                if (this.menu.getCarried().isEmpty()) {
                    if ($$2) {
                        this.slotClicked($$3, $$7, mouseButtonEvent.button(), ContainerInput.CLONE);
                    } else {
                        boolean $$8 = $$7 != -999 && mouseButtonEvent.hasShiftDown();
                        ContainerInput $$9 = ContainerInput.PICKUP;
                        if ($$8) {
                            this.lastQuickMoved = $$3 != null && $$3.hasItem() ? $$3.getItem().copy() : ItemStack.EMPTY;
                            $$9 = ContainerInput.QUICK_MOVE;
                        } else if ($$7 == -999) {
                            $$9 = ContainerInput.THROW;
                        }
    ...
    public boolean mouseDragged(@NotNull MouseButtonEvent mouseButtonEvent, double dragX, double dragY) {
        Slot slot = this.findSlot(mouseButtonEvent.x(), mouseButtonEvent.y());
        ItemStack itemStack = this.menu.getCarried();
        if (this.isQuickCrafting && slot != null && !itemStack.isEmpty() && (itemStack.getCount() > this.quickCraftSlots.size() || this.quickCraftingType == 2) && AbstractContainerMenu.canItemQuickReplace(slot, itemStack, true) && slot.mayPlace(itemStack) && this.menu.canDragTo(slot)) {
        ...
        if (this.doubleclick && $$1 != null && $$0.button() == 0 && this.menu.canTakeItemForPickAll(ItemStack.EMPTY, $$1)) {
                ...
                if (!this.lastQuickMoved.isEmpty()) {
                    for(Slot $$6 : this.menu.slots) {
                        if ($$6 != null && $$6.mayPickup(this.minecraft.player) && $$6.hasItem() && $$6.container == $$1.container && AbstractContainerMenu.canItemQuickReplace($$6, this.lastQuickMoved, true)) {
                            this.slotClicked($$6, $$6.index, $$0.button(), ContainerInput.QUICK_MOVE);
                        }
                    }
                }
            } else {
                this.slotClicked($$1, $$5, $$0.button(), ContainerInput.PICKUP_ALL);
            }
            ...
            if (this.isQuickCrafting && !this.quickCraftSlots.isEmpty()) {
                ...
                for(Slot $$8 : this.quickCraftSlots) {
                    this.slotClicked($$8, $$8.index, AbstractContainerMenu.getQuickcraftMask(1, this.quickCraftingType), ContainerInput.QUICK_CRAFT);
                }
                ...
                if (this.minecraft.options.keyPickItem.matchesMouse($$0)) {
                    this.slotClicked($$1, $$5, $$0.button(), ContainerInput.CLONE);
                } else {
                    ...
                    this.slotClicked($$1, $$5, $$0.button(), $$9 ? ContainerInput.QUICK_MOVE : ContainerInput.PICKUP);
                }
            }
        }
    ...
    protected void slotClicked(Slot slot, int slotId, int mouseButton, ContainerInput containerInput) {
        ...
        this.minecraft.gameMode.handleContainerInput(this.menu.containerId, slotId, mouseButton, containerInput, this.minecraft.player);
    }
        ...
        if (super.keyPressed(keyEvent)) {
            ...
            if (this.hoveredSlot != null && this.hoveredSlot.hasItem()) {
                if (this.minecraft.options.keyPickItem.matches(keyEvent)) {
                    ...
                    this.slotClicked(this.hoveredSlot, this.hoveredSlot.index, keyEvent.hasControlDown() ? 1 : 0, ContainerInput.THROW);
                }
            }
        ...
        if (this.menu.getCarried().isEmpty() && this.hoveredSlot != null) {
            ...
            for(int $$1 = 0; $$1 < 9; ++$$1) {
                if (this.minecraft.options.keyHotbarSlots[$$1].matches($$0)) {
                    this.slotClicked(this.hoveredSlot, this.hoveredSlot.index, $$1, ContainerInput.SWAP);
                    return true;
                }
            }
        }
