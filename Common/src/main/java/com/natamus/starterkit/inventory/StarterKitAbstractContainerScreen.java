...
public abstract class StarterKitAbstractContainerScreen<T extends AbstractContainerMenu> extends Screen implements MenuAccess<T> {
    ...
    public void render(@NotNull PoseStack poseStack, int i, int j, float f) {
        ...
        if (!this.snapbackItem.isEmpty()) {
            ...
            o = this.snapbackEnd.y - this.snapbackStartY;
    ...
    private void recalculateQuickCraftRemaining() {
        ...
        if (!itemStack.isEmpty() && this.isQuickCrafting) {
            if (this.quickCraftingType == 2) {
                ...
                int i;
                for(Iterator var2 = this.quickCraftSlots.iterator(); var2.hasNext(); this.quickCraftingRemainder -= itemStack2.getCount() - i) {
        ...
        if (this.doubleclick && slot != null && i == 0 && this.menu.canTakeItemForPickAll(ItemStack.EMPTY, slot)) {
            ...
            if (this.clickedSlot != null && this.minecraft.options.touchscreen().get()) {
                ...
                if (this.minecraft.options.keyPickItem.matchesMouse(i)) {
                    ...
                    bl2 = l != -999 && (InputConstants.isKeyDown(Minecraft.getInstance().getWindow().getWindow(), 340) || InputConstants.isKeyDown(Minecraft.getInstance().getWindow().getWindow(), 344));
                    if (bl2) {
                        this.lastQuickMoved = slot != null && slot.hasItem() ? slot.getItem().copy() : ItemStack.EMPTY;
                    }
